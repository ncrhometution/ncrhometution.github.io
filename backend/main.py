...
#
# Queries the table for the highest existing number
# with the given prefix, then returns prefix + next number
# zero-padded to 3 digits.
#
# ============================================================

def generate_lead_id(table: str, prefix: str) -> str:
...
def create_student(
    ...
    record = {
        ...
        "id": generate_lead_id("students", "NCR"),
...
def create_tutor(
    ...
    record = {
        ...
        "id": generate_lead_id("tutors", "TECH"),
...
#
# Admin only. Creates many leads at once (Excel/CSV import).
# Body: { "type": "tutor"|"student", "leads": [ {...}, ... ] }
# Each lead may include: name, mobile_no, email, course, subject,
# qualification, experience_years, payment, payment_type,
# preferred_mode, language, country, address, city, timezone.
# IDs are auto-generated (NCR### / TECH###).
#
# ============================================================
...
def admin_bulk_create_leads(
    ...
    if data.type not in ("tutor", "student"):
    ...
    for item in data.leads:
        ...
        record = {
            "id": generate_lead_id(table_name, prefix),
            "type": data.type,
            "name": name,
            "mobile_no": str(item.get("mobile_no") or "").strip() or None,
            "email": str(item.get("email") or "").strip() or None,
            "course": str(item.get("course") or "").strip() or None,
            ...
            "payment_type": str(item.get("payment_type") or "per_hour").strip(),
            "preferred_mode": str(item.get("preferred_mode") or "online").strip(),
            "language": str(item.get("language") or "English").strip(),
            "country": str(item.get("country") or "India").strip(),
            "address": str(item.get("address") or "").strip() or None,
            "city": str(item.get("city") or "").strip() or None,
            "timezone": str(item.get("timezone") or "Asia/Kolkata").strip(),
            "status": "active",
            "joined_date": now,
            "created_at": now,
            "updated_at": None,
    ...
    if not records:
        ...
        raise HTTPException(
            status_code=400,
            detail="No valid leads found in the uploaded data"
        )
    ...
    return {
        ...
        "message":
            f"{len(records)} {data.type}(s) created",
